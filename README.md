...
# from PIL import Image, ImageTk # If you want to use an image
# import time
...
def splash_window():
    ...
    # Example with text:
    splash_label = tk.Label(splash_root, text="BamBoom! Drum Sequencer program is loading...", font=("COURIER NEW", 20))
    splash_label.pack(padx=20, pady=20)

    # Example with an image:
    image = Image.open("/Users/mbp-apple-m1/Desktop/BamBoom.png")
    ...
    position_right = int(splash_root.winfo_screenwidth()/2 - window_width/4)
    position_down = int(splash_root.winfo_screenheight()/4 - window_height/4)
    splash_root.geometry("+{}+{}".format(position_right, position_down))
...
# splash_root.after(3000, main_window) # Delay in milliseconds
...
def draw_grid(clicks, beat, actives):
    boxes = []
    left_box = pygame.draw.rect(screen, green, [0, 0, 200, HEIGHT - 200], 5)
    bottom_box = pygame.draw.rect(screen, green, [0, HEIGHT - 200, WIDTH, 200], 5)
    ...
    clap_text = label_font.render('Clap', True, colors[actives[2]])
    screen.blit(clap_text, (30, 230))
    hi_hat_text = label_font.render('Hi Hat', True, colors[actives[3]])
    screen.blit(hi_hat_text, (30, 330))
        ...
        for j in range(instruments):
            ...
            rect = pygame.draw.rect(screen, color,
                                    [i * ((WIDTH - 200) // beats) + 205, (j * 100) + 5, ((WIDTH - 200) // beats) - 10,
                                     90], 0, 3)
            pygame.draw.rect(screen, white, [i * ((WIDTH - 200) // beats) + 200, j * 100, ((WIDTH - 200) // beats), 100],
                             5, 5)
            pygame.draw.rect(screen, light_brown,
                             [i * ((WIDTH - 200) // beats) + 200, j * 100, ((WIDTH - 200) // beats), 100],
                             2, 5)
            boxes.append((rect, (i, j)))
    active = pygame.draw.rect(screen, gold,
...
while run:
    ...
    # drawing lower menu
    play_pause = pygame.draw.rect(screen, brown, [50, HEIGHT - 150, 200, 100], 0, 5)
    ...
    bpm_rect = pygame.draw.rect(screen, light_brown, [300, HEIGHT - 150, 200, 100], 5, 5)
    bpm_text = medium_font.render('Beats Per Minute', True, white)
    screen.blit(bpm_text, (308, HEIGHT - 130))
    bpm_text2 = label_font.render(f'{bpm}', True, white)
    screen.blit(bpm_text2, (370, HEIGHT - 100))
    bpm_add_rect = pygame.draw.rect(screen, light_brown, [510, HEIGHT - 150, 48, 48], 0, 5)
    bpm_sub_rect = pygame.draw.rect(screen, light_brown, [510, HEIGHT - 100, 48, 48], 0, 5)
    add_text = medium_font.render('+5', True, white)
    screen.blit(add_text, (520, HEIGHT - 140))
    sub_text = medium_font.render('-5', True, white)
    screen.blit(sub_text, (520, HEIGHT - 90))
    # beats per loop buttons
    beats_rect = pygame.draw.rect(screen, light_brown, [600, HEIGHT - 150, 200, 100], 5, 5)
    beats_text = medium_font.render('Beats In Loop', True, white)
    screen.blit(beats_text, (612, HEIGHT - 130))
    beats_text2 = label_font.render(f'{beats}', True, white)
    screen.blit(beats_text2, (670, HEIGHT - 100))
    beats_add_rect = pygame.draw.rect(screen, light_brown, [810, HEIGHT - 150, 48, 48], 0, 5)
    beats_sub_rect = pygame.draw.rect(screen, light_brown, [810, HEIGHT - 100, 48, 48], 0, 5)
    ...
    # clear board button
    clear = pygame.draw.rect(screen, gold, [1150, HEIGHT - 150, 200, 100], 0, 5)
    play_text = label_font.render('Clear Board', True, white)
    screen.blit(play_text, (1160, HEIGHT - 130))
    # save and load buttons
    save_button = pygame.draw.rect(screen, light_brown, [900, HEIGHT - 150, 200, 48], 0, 5)
    save_text = label_font.render('Save Beat', True, white)
    screen.blit(save_text, (920, HEIGHT - 140))
    load_button = pygame.draw.rect(screen, light_brown, [900, HEIGHT - 98, 200, 48], 0, 5)
...
if __name__ == "__main__":
    # Example usage:
    image_path = "/Users/mbp-apple-m1/Desktop/BamBoom.png"  # Replace with your image path
    splash_duration = 3000  # Display for 3 seconds
    show_splash_screen(image_path, splash_duration)
    main()
```
